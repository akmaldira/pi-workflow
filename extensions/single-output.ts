/**
 * Single output handling — manages file-based output for subagent runs,
 * supporting "inline" and "file-only" output modes.
 */

import * as fs from "node:fs";
import * as path from "node:path";
import type { Message } from "@earendil-works/pi-ai";
import type { OutputMode, SavedOutputReference } from "./types.ts";
...
function hasMutationToolCapability(tools?: string[], mcpDirectTools?: string[]): boolean {
	const allTools = [...(tools ?? []), ...(mcpDirectTools ?? [])];
	return allTools.some((tool) => ["write", "edit", "bash"].includes(tool));
}
